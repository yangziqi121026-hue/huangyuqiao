...
class InvestmentCommittee(BaseAgent):
    ...
    def build_user_prompt(self, market_data: Dict, prev_outputs: Dict) -> str:
        ...
        # 让"分歧点"的口径与冲突检测结论保持一致：只有判定为"方向冲突"时才允许声称
        # 基本面与技术面方向冲突；否则只能描述强弱差异/不确定性，避免与综合摘要打架。
        if conflict.get("conflict"):
            conflict_directive = (
                "【冲突判定：方向冲突】分歧点必须明确指出"
                "基本面与技术面方向冲突，并保留分歧、不得强行统一方向。"
            )
        elif conflict.get("insufficient"):
            conflict_directive = (
                "【冲突判定：方向数据不足，未构成冲突】分歧点请勿声称基本面与技术面"
                "“方向冲突”；只描述数据缺失带来的不确定性，以及各维度的强弱差异。"
            )
        else:
            conflict_directive = (
                "【冲突判定：方向不冲突】分歧点请勿声称基本面与技术面“方向冲突”"
                "（例如一方为中性时即不构成方向冲突）；只描述各维度的强弱差异或不确定性。"
            )
        return (
            ...
            "- 看多理由：（凝练 1-3 条）\n"
            "- 看空理由：（凝练 1-3 条）\n"
            "- 分歧点：（以上方【冲突判定】为准：仅当判定为方向冲突时才可表述为"
            "基本面与技术面方向冲突；否则只描述强弱/不确定性分歧，不得声称方向冲突）\n"
            "- 风险等级：（低 / 中 / 高 / 不足以判断）\n"
